...
WHOLE_CLONE_ID_SUFFIX____ALT = ['0']
...
HALF_OFFSET____ALT = (0.25, -16/10*1.5+2)
HALF_MOVE____ALT = (0, 16)
HALF_SCALE____ALT = (0.55, (1+0.2)*0.8)
...
HALF_CLONE_ID_SUFFIX____ALT = ['04', '06']
...
HALFHR_CLONE_ID_SUFFIX____ALT = ['08', '02']
...
QUARTER_CLONE_ID_SUFFIX____ALT = ['07','01','09','03']
...
for obj in all_shapes():
    ...
    if isinstance(label, str) and (label.startswith('consonant_') or label.startswith('vowel_')):
        if label.startswith('consonant_'):
            ...
            process_object(obj, HALF_PREFIX, HALF_OFFSET____ALT, HALF_MOVE____ALT, HALF_SCALE____ALT, HALF_CLONE_MOVE, HALF_CLONE_OFFSETS, HALF_CLONE_ID_SUFFIX)
            ...
            process_object(obj, WHOLE_PREFIX, WHOLE_OFFSET, WHOLE_MOVE, WHOLE_SCALE, WHOLE_CLONE_MOVE, WHOLE_CLONE_OFFSETS, WHOLE_CLONE_ID_SUFFIX____ALT)
            process_object(obj, HALF_PREFIX, HALF_OFFSET, HALF_MOVE, HALF_SCALE, HALF_CLONE_MOVE, HALF_CLONE_OFFSETS, HALF_CLONE_ID_SUFFIX)
            process_object(obj, HALFHR_PREFIX, HALFHR_OFFSET, HALFHR_MOVE, HALFHR_SCALE, HALFHR_CLONE_MOVE, HALFHR_CLONE_OFFSETS, HALFHR_CLONE_ID_SUFFIX____ALT)
            process_object(obj, QUARTER_PREFIX, QUARTER_OFFSET, QUARTER_MOVE, QUARTER_SCALE, QUARTER_CLONE_MOVE, QUARTER_CLONE_OFFSETS, QUARTER_CLONE_ID_SUFFIX____ALT)

    if isinstance(label, str) and label.startswith('vowelhr_'):
        process_object(obj, WHOLE_PREFIX, WHOLE_OFFSET, WHOLE_MOVE, WHOLE_SCALE, WHOLE_CLONE_MOVE, WHOLE_CLONE_OFFSETS, WHOLE_CLONE_ID_SUFFIX)
        # 执行第一阶段 (Half)
        process_object(obj, HALFHR_PREFIX, HALFHR_OFFSET, HALFHR_MOVE, HALFHR_SCALE, HALFHR_CLONE_MOVE, HALFHR_CLONE_OFFSETS, HALFHR_CLONE_ID_SUFFIX)
        process_object(obj, HALF_PREFIX, HALF_OFFSET____ALT, HALF_MOVE____ALT, HALF_SCALE____ALT, HALF_CLONE_MOVE, HALF_CLONE_OFFSETS, HALF_CLONE_ID_SUFFIX____ALT)
