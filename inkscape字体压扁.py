...
HALF_OFFSET___SMALLER = (0.25, -16/10*1.5+2)
HALF_MOVE___SMALLER = (0, 16)
HALF_SCALE___SMALLER = (0.55, (1+0.2)*0.8)
...
QUARTER_CLONE_OFFSETS = [
    ...
    (7, 0),
    (7, 6.5),
    (-(0.125-0.25)-8*0.08, -(-0.1-0.25)-8*0.08+8-1)
]
...
for obj in all_shapes():
    label = obj.svg_get('inkscape:label')
    if isinstance(label, str) and label.startswith('consonant_'):
        process_object(obj, WHOLE_PREFIX, WHOLE_OFFSET, WHOLE_MOVE, WHOLE_SCALE, WHOLE_CLONE_MOVE, WHOLE_CLONE_OFFSETS, [''])
        process_object(obj, HALF_PREFIX, HALF_OFFSET___SMALLER, HALF_MOVE___SMALLER, HALF_SCALE___SMALLER, HALF_CLONE_MOVE, HALF_CLONE_OFFSETS, '46')
        process_object(obj, HALFHR_PREFIX, HALFHR_OFFSET, HALFHR_MOVE, HALFHR_SCALE, HALFHR_CLONE_MOVE, HALFHR_CLONE_OFFSETS, '82')
        process_object(obj, QUARTER_PREFIX, QUARTER_OFFSET, QUARTER_MOVE, QUARTER_SCALE, QUARTER_CLONE_MOVE, QUARTER_CLONE_OFFSETS, '71935')
    if isinstance(label, str) and label.startswith('vowel_'):
        process_object(obj, WHOLE_PREFIX, WHOLE_OFFSET, WHOLE_MOVE, WHOLE_SCALE, WHOLE_CLONE_MOVE, WHOLE_CLONE_OFFSETS, ['0'])
        process_object(obj, HALF_PREFIX, HALF_OFFSET, HALF_MOVE, HALF_SCALE, HALF_CLONE_MOVE, HALF_CLONE_OFFSETS, '46')
        process_object(obj, HALFHR_PREFIX, HALFHR_OFFSET, HALFHR_MOVE, HALFHR_SCALE, HALFHR_CLONE_MOVE, HALFHR_CLONE_OFFSETS, ['08', '02'])
        process_object(obj, QUARTER_PREFIX, QUARTER_OFFSET, QUARTER_MOVE, QUARTER_SCALE, QUARTER_CLONE_MOVE, QUARTER_CLONE_OFFSETS, ['07','01','09','03','05'])

    if isinstance(label, str) and label.startswith('vowelhr_'):
        process_object(obj, WHOLE_PREFIX, WHOLE_OFFSET, WHOLE_MOVE, WHOLE_SCALE, WHOLE_CLONE_MOVE, WHOLE_CLONE_OFFSETS, [''])
        # 执行第一阶段 (Half)
        process_object(obj, HALFHR_PREFIX, HALFHR_OFFSET, HALFHR_MOVE, HALFHR_SCALE, HALFHR_CLONE_MOVE, HALFHR_CLONE_OFFSETS, '82')
        process_object(obj, HALF_PREFIX, HALF_OFFSET___SMALLER, HALF_MOVE___SMALLER, HALF_SCALE___SMALLER, HALF_CLONE_MOVE, HALF_CLONE_OFFSETS, ['04', '06'])

        # 执行第二阶段 (Quarter)
        # 再次使用 target，保证 Quarter 也是从原件复制
        process_object(obj, QUARTER_PREFIX, QUARTER_OFFSET, QUARTER_MOVE, QUARTER_SCALE, QUARTER_CLONE_MOVE, QUARTER_CLONE_OFFSETS, ['7', '1', '9', '3', '5'])