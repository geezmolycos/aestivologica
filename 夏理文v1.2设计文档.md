...
構字是夏理文的表音文字部分，是全音素表音文字，每一個音素都要完整寫出。構字的基礎是部件，由若干個部件組成一個「株」，根據情況不同，一個株可以代表語音的一個音節，或者一個音素等，大部分情況下一個株和一個漢字的書寫大小相同。部件和株的關係基本上可類比於諺文的部件（字母）和音節的關係。「株」的概念不只適用於構字，它是一個通用的代表「大約一個漢字大小」完整字符的名稱。

構字部件是構字中的最小單位，不可再分。一個構字部件表示一個音素。基礎的構字部件分爲兩類，一類叫作「構枝」，包含\<CGh.>（輔音類音素），另一類叫作「構幹」，包含\<VD>（元音類音素）。各部件列舉在下面兩小節。
...
半元音 +i+ 和 +u+ 字形設計爲 ::ii:: 和 ::uu::，這是爲了與普通的輔音、元音區分開，表明它在形態學上的特殊地位。
...
最後，考慮到書寫方便，每個字形在書寫時均能使用三個筆畫以內寫出。這些字形沒有強制規定的筆順，可以參照一般中文的習慣寫法。
